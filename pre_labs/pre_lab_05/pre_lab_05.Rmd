...
**Task** Load the five CSV files. Do they have the same column structure? **Answer** Yes, from what I can see all of these files have the same column structure. 
...
**Task** Load the Maryland 2020 county population data **Answer** I loaded the Maryland 2020 county population data below.
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are now 24 rows.
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **Answer** It says we are using the "join_by" function to join the "county" column. 
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** The county with the highest 18+ Black population is Prince George's county with 60.9%. The percentage of Democrats in that county is 78.3%. 
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** Now, the county with the highest percentage of 18+ Asian population is Howard county at 19.4%, with 52.4% of that population being Democrats. In comparison to previous results, the population percentage is significantly smaller, with 19% being the highest percentage. Wheras, the previous results' highest percentage was a little over 60%. 
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **Answer** The datatype of ZCTA5N is a <dbl> which indicates that it is a numerical value. 
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **Answer** Now, the column has been changed to a character string, or <chr>.
