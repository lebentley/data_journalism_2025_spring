...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer** The columns include jurisdictions, as well as presidential candidates. However, the only anser in the Sharon E. Harris column is zero. They are also all categorized as character strings. 
...
**Task** Run the following code to extract the table we want, clean up the names and remove the total row. Look at the rename() function - what do you think the numbers 2 through 6 are referring to? **Answer** I think that the two and six could potentially referring the the fact that harris is the 2nd column and kennedy is the sixth. So, the numbers would demonstrate where the columns are/what number they are within the data frame. 
...
**Task** Run the following code in the gray-colored codeblock below to find the <li> tags. What does the output suggest about how many <ul> tags there are in the HTML source? **Answer** The output suggests that there are 20 <ul> tags in the HTML source. 
...
**Task** Run the following code in the gray-colored codeblock below to find the text inside <li> tags. Describe the output: **Answer** the output is showing that there are 54 <li> tags within the data. The tags contain categories such as Representatives, Leadership, Committees, etc. 
...
### Task 11: Turn the contents of the <li> and <a> tags into a dataframe **Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **Answer** The output is a dataframe with name, type, and value. 
