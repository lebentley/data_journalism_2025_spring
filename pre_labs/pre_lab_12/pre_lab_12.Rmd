...
**Task** Run the following code and describe the data frame it outputs. **Answer** It gives me a data frame with the columns date, title, url, text. I believe the urls are all links to different press releases or articles where we are getting the words from, and the text is the actual article. 
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** The output is listing all the possible outcomes from the list of words. In other words, it looks like it combined words that had doubles, such as "cat" and made it into one column. But overall, the code is creating columns of those titles. 
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** The output gives me a list titled "unique_words." They are each numbered and it says there are 677,425 entries total. Although, some of the words listed seem like they are extremely commonly repeated/used words. 
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** The graphic this produces shows a count of unique words found in Cardin releases. I do think this graphic is interesting, but I am not entirely sure why it seems to be including words that don't seem to be unqiue.
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** Yes, this is more interesting than before because the words seem more unique. For example, I am curious of the context of some of the top words such as "health" and "support."
...
**Task** Run the following code and describe the results. **Answer** The results still show a list of the top words, along with a number for how many times they were said, and a percentage indicating how often that was. The top words are "baltimore," "support," and "funding."
...
**Task** Run the following code and describe the results. **Answer** Now the output has a main column called "bigram." The top results in this column are "amount included," "healthcare," and "prince george's." The other two columns are the same categories as before. 
...
**Task** Run the following code and describe the results. **Answer** Now there is a data frame with the top two-word phrases. The top three are "health care," "amount awarded," and "purpose funds."
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **Answer** The first result has a larger variance among words that were tracked, while the second result repeats the word "drive" for 4/5 answers. I think the first one would be more helpful for the press release data, because it shows a larger variance of words with "drive" in it, but doesn't limit itself to only tracking words that appear exactly as that. 
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **Answer** The output looks like it is back to tracking top words used. I think that compared to the results above, this data is much more useful. It provides a much larger list of common words, giving a journalist potentially more opportunity for analysis/storytelling. 
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** The actual words the data frame is tracking seems correct to me, but I am a little skeptical of tracking words as definitively either positive or negative. The reason I say this is because the connotation of these words can sometimes based on context. Even if a word is mostly negative, you can likely not guarantee that every time it is said in an objectively negative context.
