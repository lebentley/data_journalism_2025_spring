...
**Task** Run the following code to load some campaign expenditure data focused on food-related expenses in Washington, D.C. **Answer** I ran the code to display the column.
...
**Task** Run the following code to group and count records in the recipient_name column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** A potential problem with the grouping could be that some of the same companies/stores could be catagorized as separate because of the filtering. 
...
**Task** Run the following code to group by recipient_clean and recipient_name, then filter for cities that start with W. Name three changes the refinr function made in the first 20 results. **Answer** Now, there are no groupings for committees with incorrect grammar. Also, the same committees with spelling errors are no longer grouped together to fix that. Lastly, there is no grouping for sanme committees with different/varying appreviations. 
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with A. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** Looking at the data, it seems like more of the same committees that had slight errors in data entry are grouped together. That being said, there are still some of the same committees with only grammar/capitalization differences being listed twice. 
