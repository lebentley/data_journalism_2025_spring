...
In the space below, write the data type for the "last_name" column and the "amount" columns (it's in light gray, offset by \< \>). Also offer your best guess as to what the "last_name" column and the "amount" column represent. **The data type for the "last_name" column is <chr>. The data type "amount" column is <dbl>. I think these might represent when the data is presented as words like in "last_name" or numerals, such as in "amount." **
...
In the space below, write the first and last name from the _second_ row of the data. **The first name from the second row of data is "Peggy." The last name from the second row of data is "Mukai." **
...
**I think the code below is going to reverse the data to be grouped from most recent to earliest, rather than earliest to most recent.**
...
**It changed the way the data is grouped, it is now ordered from most recently to the earliest date it started.**
...
**The data is no longer grouped by the categories "memo_text" and "amount." **
...
**The data is no longer grouped by "amount" as well as "memo_text." It is now soley organized by "memo_text."**
...
**The top result from Maryland is "Earmarked for HOGAN FOR MARYLAND INC. (C00869016)" **
