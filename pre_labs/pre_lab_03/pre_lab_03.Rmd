...
**Task** Check for problems that occurred when loading the Texas precinct results by running the following code. How many problems were there, as shown by the number of rows in the output table showing errors? What do you think the problem is that R is describing? Answer below. **Answer** I could guess that there is potentially something wrong with the the datatype not matching what it actually is. For example, in the "actual" column the datatype reads "chr." However, the data in the set are actually integers.
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** It did not show me any parsing errors this time. In the delimiter section chr is showing up as red, but I am not seeing a warning section.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answer** In the district field, the data all simply reads "N/A" for me. However, the datatype is "dbl." In the precinct field the datatype is labeled "chr" and reads "Box 1" in the row. The mail column also is labeled "dbl" but still reads "N/A" for the actual data. 
...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **Answer**: There are 402,345 results where the results for mail are N/A. The implication could be that mail votes either weren't recorded or are irrelevant in this set, but I'm not completely sure.
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **Answer** I think that the confimation not returned and confirmation returned undeliverable could potentially be worth exploring. 
...
**Answer** There are 14 rows in this dataset. 
...
**Task** Run the following codeblock to glimpse the data. What data type is the "amount" field? Answer below. **Answer** The amount field says the datatype are charachets, but the actual data is displaying numbers. 
...
**Answer** One problem could be that in the zip section it says the datatype is a character string, but the values are integers. This same problem also happened in the amount section.Another problem could be that there is not a lot of data recorded for sections middle_name, prefix, and suffix.
...
**Answer** Instead of the column being "1_linenumber," it is named "x1_linenumber."
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **Answer** It tells me that there is an error in trying to use the "summarise()" function. It also says it is an invalid type of argument.I think the error means that you cannot summarise the total amount. In other words, you would have to find the sum of the total amound, and then summarise it. 
...
**Task** Run the following codeblock to attempt to change the datatype for the amount field to a number. What is the new data type (three letter code) for amount? Answer below. **Answer** The new data type for amount is dbl. 
```{r}
# cleaning function
cleaned_conowingo <- conowingo |>
  clean_names() |> 
...
**Task** Run the following codeblock to add up the amount of all contributions in this data. What is the total? Answer below. **Answer** The new total is 226.
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. **Answer** There are two rows named "SA11AI."
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. Add a description of what this code does to your reference notebook. **Answer** The new dataframe has 13 rows. 
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **Answer** Zipcodes that had the additional four numbers are now just reduced to five numbers, potentially making your dataset less distinct.
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **Answer** I see one misspelling remaining, as it says "Conowingoo."
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field using both the exact match method and the str_detect() method. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **Answer** I no longer see any misspellings of the word in my dataset. 
