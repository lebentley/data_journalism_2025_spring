...
**Answer** Mutating is creating a new column called UNAF_DIFF in "md_voters", where it takes the difference between unaffiliated voters for 2024 and unaffiliated voters for 2022. Then, it is creating a dataframe called "maryland_top_counties" using "md_voters." It is arranging the difference column in descending order and selecting just the county and difference columns. The last line is showing just the first ten results, or the top-ten.
```{r}
colnames(md_voters)
```
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** I see a white space which I believe is the black canvas that will hold our chart.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** I see a bar chart with count and county - and the scale goes up to 15000. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The counties are now arranged in ascending order. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** It is still a bar chart but the county different is now on the y-axis, while count is on the x. The bars are also facing out sideways rather than up. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The labels on each county have changed just slightly.
...
**Task** Run the following code. **Answer** I installed the packages. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer** The chart now has more colors, which makes it more clear to read the data and see the differences between each county. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** A title was added to the chart, and the "count" on the x-axis was renamed to "net change." Also, a source was added in a caption at the bottom of the chart. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** I see a list with two columns, one with "date" and one with "total_contributions." There are 91 rows total.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** I see a chart with a fluctuating line graph. The x-axis is labled "date" and the y-axis is labeled "total_contributions."
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** It is nearly the same, however, now the dates in the "date" column are specified by days.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The days have been reformatted so that they fit on the chart better and look more aesthetically pleasing. 
