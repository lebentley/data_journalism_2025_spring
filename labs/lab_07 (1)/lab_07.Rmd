...
```{r}
joined_data <- cvr |>
  inner_join(cvr, by = "precinct")

joined_data <- joined_data |>
  filter(!is.na(governor_lt_governor), !is.na(comptroller)) %>%
  filter(governor_lt_governor != "undervote", comptroller != "undervote") %>%
  mutate(ticket_split = governor_lt_governor != comptroller)

# Step 4: Count voters and splitters per ZIP
ticket_stats_by_zip <- cvr_clean %>%
  group_by(zipcode) %>%
  summarise(
    total_voters = n(),
    ticket_splitters = sum(ticket_split),
    split_pct = round(100 * ticket_splitters / total_voters, 2)
  ) %>%
  arrange(desc(split_pct))
