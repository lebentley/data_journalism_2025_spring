...
```{r}
frederick_cvr <- read_csv("~/Documents/GitHub/data_journalism_2025_spring/labs/lab_07 (1)/data/frederick_cvr.csv")
...
```{r}
names (frederick_cvr)
names(frederick_precinct)
glimpse(frederick_cvr)
```
```{r}
joined_data <- left_join(frederick_cvr, frederick_precinct, by ="precinct") 

joined_data <- joined_data |>
  mutate(split_gov_compt = governor_lt_governor != comptroller &
                      governor_lt_governor != "undervote" &
                      comptroller != "undervote"
  )

gov_compt_split_summary <- joined_data %>%
  count(split_gov_compt) %>%
  mutate(percent = n / sum(n) * 100)

# Consistency across all 5 races
cvr_joined <- joined_data %>%
  mutate(
    all_five_match = governor_lt_governor == comptroller &
                     governor_lt_governor == attorney_general &
                     governor_lt_governor == u_s_senator &
                     governor_lt_governor == representative_in_congress_6 &
                     !governor_lt_governor %in% c("undervote", "overvote") )

all_five_match_summary <- joined_data %>%
  count(all_five_match) %>%
  mutate(percent = n / sum(n) * 100)
